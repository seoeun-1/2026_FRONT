...
console.log(`안녕 ${a}`) // 문자열 템플릿은 `백틱을 감싼 자료 안에 ${} 이용하여 코드 포함
// 3. 이스케이프(제어) 문자 , \ 백슬래시
console.log('\\안녕') // '\' 출력
console.log('\n안녕') // '\n' 줄바꿈
console.log('\'안녕') // 문자열 감싼 기호 화 그자체 기호 구분
console.log('\t안녕') // '\t' 들여쓰기
...
console.log(10>3) // 논리연산자의 반환값
 ...
 // [4] undefined, null , 컴푸터는 0 vs null(참조가 없다) vs undefined(정의가 없다) 모두 다르게 취급
 ...
 // [5] 배열 , 순서대로 나열
 // [ ] 대괄호 안에, 쉽표 이용하여 여러개 자료들을 *하나의 자료* 표현 , *중첨 가능*
 // 인덱스(색인) : [ ] 해당 배열네 저장괸 자료들의 순서번호 ,  0번 부터 시작
...
// 
let 요소값1 = 배열변수[4] // (1) 4변 인덱스 요소 꺼내고 변수에 저장
let 요소값2=  요소값[1] // (2) 1번 인덱스 요소 꺼내고 변수에 저장
console.log(요소값2)

// 2. 배열수정, 변수명 = 새로운배열 *변수(자료) vs 배열(자료)*
배열변수 = ['유재석' , '강호동'] // 변수는 하나의 자료 저장하기 때문에 새로운 대입이 들어오면 기존 대입을 사라진다.
console.log(배열변수)
배열변수[0] = '유재석2' // 0변 자리에 유새석을 유재석2 변경한다.
console.log(배열변수)
...
// .include(찾을값) , 만약에 찾을값이 존재하면 true 없으면 false 반환
let 존재여부 = 배열변수.includes('강호동')
console.log(존재여부)

// [6] 타입 반환 , 3 vs "3" 숫자3 과 문자3 다르다 , 실무는 JS만으로 개발x 외부언어/자료 혼합[o]
...
// 2. 직접 타입 반환
console.log(Number("10")) // 문자열 10 -> 숫자형 10으로 반환
console.log(Number("10A")) // NaN(NOT A NUMBER) , 10A -> 숫자형으로 반환 불가능
console.log(parseInt("10")) // 문자열 10 -> 숫자(정수) 10으로 반환
console.log(parseFloat("3.14")) // 문자열 3.14 -> 숫자(실수) 3.14으로 반환
console.log(String(10)) // 숫자형 10 -> 문자열 10으로 반환
console.log(10 + "")
// **
...
// 2. 연결연산자 : +연결연산자
// 3. 비교연산자 : >초과 <미만 >=이상 <=이하 ==같아 !=(값)같지않다. ===(값/타입)같다 !==(값/타입)같지않다.
// 4. 논리연산자 : &&이면서 ||이거나 !부정

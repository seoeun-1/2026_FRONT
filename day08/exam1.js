...
// 여러개 자료들을 저장하는 방법 : 1.배열[ ] 인덱스 식별 2. 객체 { } 속성명 식별
// 서로 다른 자료들의 의미/용도 다르면 객체 권장 , 같으며 배열 권장
...
let values = Object.values( book ) // 객체내 모든 속성값 (배열) 변환
for( let index = 0 ; index <= values.lenght-1 ; index++ ){ // 인덱스가 0부터 마지막 인덱스까지 1씨 증가 반복
...
// console.log( user.job-title ) // 주의할점 : 속성명에 특수문자가 있거나ㅏ 변수로 정의한 경우에는 .점이 아닌 [ ] 사용
...
let 속성명목록 = Object.values( country ); console.log( 속성값목록 )
...
for( let index = 0 ; index <= 속성명들.ㅣ두홋-1 ; ㅑㅜㅇㄷㅌ++){
    if( inventory[ 속성명 ] < 10){
        console.log( 속성명 )
    }
}
